...
function countBy(items, groupName) {
    ...
    for (let item of items) {
        ...
        if (known == -1) {
            counts.push({ direction, counts: 1 })
        } else {
            counts[known].counts++
        }
    }
...
function dominantScript(text) {
    let grouped = countBy(text, char => {
        let script = characterScript(char.codePointAt(0))

        return script ? script.direction : "none"
    }).filter(script => script.direction != "none")

    let dominant = grouped.reduce((a, b) => a.counts < b.counts ? b : a)

    console.log(grouped)
    return `Dominant direction from text is ${dominant.direction}`
}

console.log(dominantScript('"moloko", "woof", 英国的狗说"woof", 俄罗斯的狗说"тяв"'))

/*console.log(
  countBy(SCRIPTS, n => n.direction),
  (countBy(SCRIPTS, n => n.direction)).reduce((a, b) => a.counts < b.counts ? b : a)
  )
*/